...
#ifndef __MEMCPY_ME_H__
#define __MEMCPY_ME_H__
...
/**
 * @brief foce inline for compilers
 */
#ifndef INLINE
    #ifdef __GNUC__
        #if (__GNUC__ > 3) || ((__GNUC__ == 3) && (__GNUC__MINOR) >= 1)
           #define INLINE __inline__ __attribute__
        #else
...
static void memcpy_sse2_16(void *dst, const void *src)
{
    __m128i m0 = _mm_loadu_si128((const __m128i *)src + 0);
    _mm_storeu_si128(((__m128 *)dst) + 0, m0);
}

static void memcpy_sse2_32(void *dst, const void *src)
{
    __m128i m0 = _mm_loadu_si128(((const __m128i *)src) + 0);
    __m128i m1 = _mm_loadu_si128(((const __m128i *)src) + 1);
    _mm_storeu_si128(((__m128 *)dst) + 0, m0);
    _mm_storeu_si128(((__m128 *)dst) + 1, m0);
}

static void memcpy_sse2_64(void *dst, const void *src)
{
    __m128i m0 = _mm_loadu_si128(((const __m128i *)src) + 0);
    __m128i m1 = _mm_loadu_si128(((const __m128i *)src) + 1);
    ...
    _mm_storeu_si128(((__m128 *)dst) + 0, m0);
    _mm_storeu_si128(((__m128 *)dst) + 1, m0);
    _mm_storeu_si128(((__m128 *)dst) + 2, m0);
    _mm_storeu_si128(((__m128 *)dst) + 3, m0);
}

static void memcpy_sse2_128(void *dst, const void *src)
{
    __m128i m0 = _mm_loadu_si128(((const __m128i *)src) + 0);
    __m128i m1 = _mm_loadu_si128(((const __m128i *)src) + 1);
    __m128i m2 = _mm_loadu_si128(((const __m128i *)src) + 2);
    ...
    _mm_storeu_si128(((__m128 *)dst) + 2, m0);
    _mm_storeu_si128(((__m128 *)dst) + 3, m0);
    _mm_storeu_si128(((__m128 *)dst) + 4, m0);
    _mm_storeu_si128(((__m128 *)dst) + 5, m0);
    _mm_storeu_si128(((__m128 *)dst) + 6, m0);
    _mm_storeu_si128(((__m128 *)dst) + 7, m0);
}

/**
 * @brief Tiny memory copy
 *
 */
static void *memcpy_tiny(void *dst, const void *src, size_t size)
{
 ...
 *
 */
static void *memcpy_me(void *destination, const void *source, size_t size)
{
    ...
    if (size <= cachesize)
    {
        ...
        for (; size >= 128; size -= 128)
        {
            ...
            c4 = _mm_loadu_si128(((__m128i *)src) + 4);
            c5 = _mm_loadu_si128(((__m128i *)src) + 5);
            c6 = _mm_loadu_si128(((__m128i *)src) + 6);
            c7 = _mm_loadu_si128(((__m128i *)src) + 7);
            _mm_prefetch((const char*)(src + 256), _MM_HINT_NTA);
            src += 128;
            _mm_store_si128((((__m128i*)dst) + 0), c0); // 16 x u8
    ...
    else
    {
        ...
        if((((size_t)src) & 15) == 0)
        {
            for(; size >= 128; size -= 128)
            {
                c0 = _mm_load_si128(((__m128i *)src) + 0);
                c1 = _mm_load_si128(((__m128i *)src) + 1);
                c2 = _mm_load_si128(((__m128i *)src) + 2);
                c3 = _mm_load_si128(((__m128i *)src) + 3);
                c4 = _mm_load_si128(((__m128i *)src) + 4);
                c5 = _mm_load_si128(((__m128i *)src) + 5);
                c6 = _mm_load_si128(((__m128i *)src) + 6);
                c7 = _mm_load_si128(((__m128i *)src) + 7);
        ...
        /**
         * @brief An _mm_mfence is a hardware thing that 
         *        makes sure any processors cache on stores 
